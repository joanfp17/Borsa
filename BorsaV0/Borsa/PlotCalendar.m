...
PlotCalendar[ timeSeries_, opts: OptionsPattern[]] :=
    Module[ {data, plotData, years, dataRange, filterOutliers},
        ...
        years = If[ OptionValue["Years"] === All,
                    DeleteDuplicates[DateValue[#, "Year"]&/@data[[All,1]]],
                    OptionValue["Years"]
                ];
...
monthstart[month_,year_] :=
    DayName[{year,month,1}]/.dayindex;

dayspermonth[month_,year_] :=
    DayCount[{year,month,1},DatePlus[{year,month,1},{{1,"Month"}}]];
...
monthgrid[month_,year_] :=
    Module[ {cd,cds,p1,p2,p3,p4,p5,p6,p7,p8,shift},
...
monthNoDataIndicator[month_,year_] :=
    Module[ {cd,cds,shift},
        shift = startingweek[month,year];
        cd = monthlayout[month,year];
        cds = Position[cd,x_/;x!=""];
...
yeargrid[year_,opts:OptionsPattern[Show]] :=
    Show[Table[monthgrid[i,year],{i,1,12}],opts];



(* dataRaw must contain: {DateObject, Color, Value for Tooltip} *)
monthHeatMap[dataRaw_, {year_, month_}, opts: OptionsPattern[PlotCalendar]] :=
    Module[ {data, rectposition, colorBlocks},
  (* select data that is in that month *)
        data = Select[ dataRaw, #[[1,1,1]] === year && #[[1,1,2]] === month &];
        (*calculate xy postion for the each date*)
        rectposition[{y_,m_,d_}] :=
            Module[ {lx,ly},
...
plotYear[dateAndColor_, year_, opts:OptionsPattern[PlotCalendar]] :=
    Show[
...
plotYear[dateAndColor_, years_List, opts:OptionsPattern[PlotCalendar]] :=
    Column[
