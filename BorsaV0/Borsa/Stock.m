...
actualizeStocks::usage = "actualizeStocks[] actualizes all stocks in Ticker from yahoo"
...
Stock[s_String] :=
    Module[ {symbol, dividends, splits},
        ...
        If[ Head@dividends =!= RObject,
            dividends = {}
        ];
        Quiet[splits = REvaluate["getSplits(\"" <> s <> "\")"]];
        If[ Head@splits =!= RObject,
            splits = {}
        ];
...
Stock[s_String,ohlcv_List,adjp_List,div_List,split_List][p_String] :=
    Which[
     ...
     weekPrices[ohlcv],
     p === "Actualize",
     actualize[s,ohlcv,adjp,div,split],
     True,
...
Stock[s_String,ohlcv_List,adjp_List,div_List,split_List][ld_List] :=
    Module[ {ldat = AbsoluteTime[ld]},
        Stock[s,
        If[ AbsoluteTime[#[[1]]] <= ldat,
            #,
            Nothing
        ] & /@ ohlcv,
        If[ AbsoluteTime[#[[1]]] <= ldat,
...
Format[Stock[s_String,ohlcv_List,adjp_List,div_List,split_List],StandardForm] :=
    Panel[Column[{s,Row[{ "from  ", DateObject[ohlcv[[1,1]]], "  to  ", DateObject[ohlcv[[Length[ohlcv],1]]]}], Length[ohlcv] " bars"}],Style["Stock Object",16]]
...
actualize[s_String,ohlcv_List,adjp_List,div_List,split_List] :=
    Module[ {lastDate = (#[[1]] & /@ ohlcv)[[-1]] - {1, 0, 0},news,olds,final},
        news = Stock[s, lastDate];
        olds = Stock[s,ohlcv,adjp,div,split][lastDate-{0,0,1}];
        final = Stock[olds[[1]],Join[olds[[2]],news[[2]]],Join[olds[[3]],news[[3]]],Join[olds[[4]],news[[4]]],Join[olds[[5]],news[[5]]]];
        Put[final, "C:\\Users\\fabregas.TCMM\\Google Drive\\Borsa\\Ticker\\" <> final["Name"]];
        final
    ]
